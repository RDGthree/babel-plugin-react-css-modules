...
const getTokens = (runner, cssSourceFilePath, filetypeOptions, includePaths) => {
  const extension = cssSourceFilePath.substr(cssSourceFilePath.lastIndexOf('.')); // eslint-disable-next-line flowtype/no-weak-types
  ...
  if (extension === '.scss') {
    fileContents = _sass.default.renderSync({
...
var _default = (cssSourceFilePath, options) => {
  ...
  const plugins = [...extraPlugins, _postcssModulesValues.default, _postcssModulesLocalByDefault.default, _postcssModulesExtractImports.default, (0, _postcssModulesScope.default)({
    generateScopedName
